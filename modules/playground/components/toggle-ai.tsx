...
const ToggleAI: React.FC<ToggleAIProps> = ({
  ...
  return (
    <>
          ...
          <DropdownMenuItem onClick={() => onToggle(!isEnabled)} className='py-2.5 cursor-pointer'>
            <div className='flex items-center justify-between w-full'>
              <div className='flex items-center gap-3'>
                {isEnabled ? <Power className='h-4 w-4 text-muted-foreground' /> : <PowerOff className='h-4 w-4 text-muted-foreground' />}
                <div>
                  <div className='text-sm font-medium'>{isEnabled ? 'Disable' : 'Enable'} AI</div>
              ...
              <div
                className={cn(
                  ...
                  isEnabled ? 'bg-zinc-900 border-zinc-900 dark:bg-zinc-50 dark:border-zinc-50' : 'bg-muted border-border'
                )}
              >
                <div
                  className={cn('w-3 h-3 rounded-full bg-background transition-all duration-200 absolute top-0.5', isEnabled ? 'left-4' : 'left-0.5')}
                />
              </div>
            </div>
          </DropdownMenuItem>
          ...
          <DropdownMenuItem onClick={() => setIsChatOpen(true)} className='py-2.5 cursor-pointer'>
            <div className='flex items-center gap-3 w-full'>
              ...
              <div>
                ...
                <div className='text-xs text-muted-foreground'>Chat with AI assistant</div>
              </div>
            </div>
          </DropdownMenuItem>
        </DropdownMenuContent>
      ...
      <AIChatSidePanel isOpen={isChatOpen} onClose={() => setIsChatOpen(false)} />
    </>
  );
};
