...
type TemplateSelectionModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: { title: string; template: 'REACT' | 'NEXTJS' | 'EXPRESS' | 'VUE' | 'HONO' | 'ANGULAR'; description?: string }) => void;
};
...
const TemplateSelectionModal = ({ isOpen, onClose, onSubmit }: TemplateSelectionModalProps) => {
  ...
  const renderStars = (count: number) => {
    return Array(5)
      .fill(0)
      .map((_, i) => <Star key={i} size={14} className={i < count ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'} />);
  };
        ...
        {step === 'select' ? (
          <>
            ...
            <div className='flex flex-col gap-6 py-4'>
              <div className='flex flex-col sm:flex-row gap-4'>
                <div className='relative flex-1'>
                  <Search className='absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 outline-none' size={18} />
            ...
            <div className='flex justify-between gap-3 mt-4 pt-4 border-t'>
              ...
              <div className='flex gap-3'>
                ...
                <Button className='bg-[#E93F3F] hover:bg-[#d03636] text-white' disabled={!selectedTemplate} onClick={handleContinue}>
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription>{templates.find(t => t.id === selectedTemplate)?.name} project configuration</DialogDescription>
            </DialogHeader>

            <div className='flex flex-col gap-6 py-4'>
              <div className='flex flex-col gap-2'>
                <Label htmlFor='project-name'>Project Name</Label>
                <Input id='project-name' placeholder='my-awesome-project' value={projectName} onChange={e => setProjectName(e.target.value)} />
              </div>
            ...
            <div className='flex justify-between gap-3 mt-4 pt-4 border-t'>
              ...
              <Button className='bg-[#E93F3F] hover:bg-[#d03636] text-white' onClick={handleCreateProject}>
                Create Project
              </Button>
            </div>
          </>
